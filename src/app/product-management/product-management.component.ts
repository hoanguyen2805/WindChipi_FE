...
export class ProductManagementComponent implements OnInit {
    ...
    this.uploadService.upload(this.currentFile).subscribe(
      event => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this.urlImage = event.body.message;
          // this.fileInfos = this.uploadService.getFiles();
        }
      },
      err => {
        this.progress = 0;
        this.message = 'Lỗi! File 1 quá nặng hoặc đã tồn tại!';
    ...
    this.uploadService.upload(this.currentFile2).subscribe(
      event => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this.urlImage2 = event.body.message;
        }
      },
      err => {
        this.progress2 = 0;
        this.message2 = 'Lỗi! File 2 quá nặng hoặc đã tồn tại!';
    ...
    this.uploadService.upload(this.currentFile3).subscribe(
      event => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this.urlImage3 = event.body.message;
        }
      },
      err => {
        this.progress3 = 0;
        this.message3 = 'Lỗi! File 3 quá nặng hoặc đã tồn tại!';
  ...
  onSubmit() {
    ...
    if (this.urlImage !== undefined) {
      newProduct.images = "http://localhost:8080/api/files/files/" + this.urlImage;
    }
    if (this.urlImage2 !== undefined) {
      newProduct.images2 = "http://localhost:8080/api/files/files/" + this.urlImage2;
    }
    if (this.urlImage3 !== undefined) {
      newProduct.images3 = "http://localhost:8080/api/files/files/" + this.urlImage3;
    }
